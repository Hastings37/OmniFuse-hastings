...
def main():
    ...
    parser.add_argument("-opt", type=str,default='options/train/train_my_latentDif_IR.yaml',help="Path to option YMAL file.")
    parser.add_argument(
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0-7)
        if resume_state is None:
            ...
            log_link_path = "./log"
            target_path = os.path.join(opt["path"]["experiments_root"], "..")

            # --- 1. 删除旧的 log (替代 os.system("rm ./log")) ---
            if os.path.exists(log_link_path):
                if os.path.islink(log_link_path):
                    os.remove(log_link_path)  # 如果是软链接，直接移除链接本身
                elif os.path.isdir(log_link_path):
                    shutil.rmtree(log_link_path)  # 如果是个真实存在的文件夹，递归删除
                else:
                    os.remove(log_link_path)  # 如果是文件，直接删除

            # --- 2. 创建新链接 ---
            try:
                # Windows 下创建指向文件夹的软链接，建议显式指定 target_is_directory=True
                os.symlink(target_path, log_link_path, target_is_directory=os.path.isdir(target_path))
            except OSError as e:
                print(f"[Warning] 无法创建软链接: {e}")
                print("提示: 在 Windows 上创建软链接通常需要 [管理员权限] 或开启 [开发者模式]。")
    ...
    for phase, dataset_opt in opt["datasets"].items():
        if phase == "train": # 图像块输入的时候的维度是 480 的；但是encoder 是128上训练的；
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader): # 计算得到其对应的avg_psnr
                    ...
                    break # 这里只是先验证一个batch 的内容看看效果怎么样；
