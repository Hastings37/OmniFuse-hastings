...
# Nonlinear Activation Free Network 不使用非线性激活函数使用SimpleGate 替代；
class NAFBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.norm1(x)
        x = x * (scale_att + 1) + shift_att # BCHW
        x = self.conv1(x).contiguous()# B2CHW
        x = self.conv2(x)# B2CHW
        x = self.sg(x) #BCHW 按照dim=1切分再点乘处理
        x = x * self.sca(x) # BCHW 通道注意力操作；
        x = self.conv3(x) # BCHW

        x = self.dropout1(x)

        y = inp + x * self.beta # 可学习参数内容 BCHW

        x = self.norm2(y)
        x = x * (scale_ffn + 1) + shift_ffn
        x = self.conv4(x)#B2CHW
        x = self.sg(x)  # BCHW
        x = self.conv5(x)
...
class ConditionalNAFNet(nn.Module):
    ...
    def forward(self, inp, cond, time):
        ...
        x = inp - cond # 输入图片和作为条件的图片
        x = torch.cat([x, cond], dim=1)

        t = self.time_mlp(time) # B D   time_dim

        B, C, H, W = x.shape
        x = self.check_image_size(x)

        x = self.intro(x) # 映射到width上
