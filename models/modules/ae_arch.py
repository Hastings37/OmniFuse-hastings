...
class TransformerUNet(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=3, ch=48, ch_mult=[1, 2, 4, 4], embed_dim=16,
        ...
        # [关键修改] 新增归一化层
        # ====================================================================
        # 作用：截断 Transformer 的残差方差累积，强制将特征分布拉回标准正态分布附近。
        # GroupNorm(1, C) 等价于 LayerNorm，但在 (B, C, H, W) 格式下更方便。
...
if __name__ == "__main__":
    ...
    model = load_model_checkpoint(model,
                                  r'E:\Recurrence\OmniFuse\experiments\OmniFuse\latent_AutoEncoder_GroupNorm\models\14000_G.pth')
    ...
    for i in range(h.shape[1]):
        ret = plot_tensor_hist(h[:, i, ...], (-2.0, 2.0), f'visual latent channel {i} after encode')

        print(f"hist channel {i} mean:{ret['mean']} std:{ret['std']} ")
