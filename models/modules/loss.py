...
def apply_gaussian_filter_Entarget(image_tensor, seg_mask, kernlen=3, nsig=3):
    ...
    processed_image[seg_mask > 0] = ((image_tensor - blurred_image) * 25 + blurred_image * 1)[
        seg_mask > 0]  # Amplify changes in masked areas
...
class fusion_modulated_loss(nn.Module):
    ...
    def forward(self, image_A, image_B, image_fused, g_label, image_Seg_Label, Fusion_Base, Seg_Label_C, int_ratio=1,
                grad_ratio=2, color_ratio=10):
...
class L_gradient(nn.Module):
    ...
    def forward(self, image_A, image_B, image_fuse, non_seg_mask=None):
        if non_seg_mask == None:
            ...
            loss = F.l1_loss(gradient_fuse_x[non_seg_mask],
                             torch.max(gradient_A_x[non_seg_mask], gradient_B_x[non_seg_mask])) + F.l1_loss(
                gradient_fuse_y[non_seg_mask], torch.max(gradient_A_y[non_seg_mask], gradient_B_y[non_seg_mask]))
            return loss
...
def loss_int_compass(image_Seg_Label, image_A_y, image_B_y, image_fused_y):
    ...
    for b in range(image_Seg_Label.shape[0]):
        ...
        extract_connected_components_A, extract_connected_components_B = compare_and_select_components(
            connected_components, image_A_y[b:b + 1, :, :, :], image_B_y[b:b + 1, :, :, :])
        ...
        if torch.mean(image_Seg_Label[b:b + 1, :, :].float()) == 0:
            ...
            loss = F.l1_loss(image_fused_y[b:b + 1, :, :, :][(image_Seg_Label[b:b + 1, :, :] > 0).unsqueeze(1)],
                             co_A_B[(image_Seg_Label[b:b + 1, :, :] > 0).unsqueeze(1)])
        # print('loss',loss)
