...
class LQGTDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #ps 这里使用的是cv2 读取的图像内容因此是 BGR 这样的格式的；
        if self.opt["phase"] != "train":
            img_GT = util.modcrop(img_GT, scale)
            # 保证图像的尺寸裁剪为可以 // scale == 0 的状态；
        ...
        if self.LR_paths:  # LR exist
            ...
            if self.opt["data_type"] == "lmdb":
                ...
                resolution = None
            img_LR = util.read_img(self.LR_env, LR_path, resolution) # BGR HWC C为1 或者是3
        else:  # down-sampling on-the-fly
            # randomly scale during training
            if self.opt["phase"] == "train":
                random_scale = random.choice(self.random_scale_list)
                H_s, W_s, _ = img_GT.shape
                # 也是作为数据增强操作的一部分，保证图像先 random_scale 缩放之后再 downscale
