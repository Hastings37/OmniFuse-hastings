...
class SDE(abc.ABC):
    ...
    @abc.abstractmethod
    def drift(self, x, t):  # 漂移项
        pass

    @abc.abstractmethod
    def dispersion(self, x, t):  # 扩散项
        pass

    @abc.abstractmethod
    def sde_reverse_drift(self, x, score, t):  # sde 逆向漂移项
        pass

    @abc.abstractmethod
    def ode_reverse_drift(self, x, score, t):  # ode 逆向漂移项
        pass

    @abc.abstractmethod
    def score_fn(self, x, t):  # score function
        pass
...
class IRSDE(SDE):
    ...
    def _initialize(self, max_sigma, T, schedule, eps=0.01):
        ...
        def cosine_theta_schedule(timesteps, s=0.008):
            ...
            # [0,T+2]
            # 从约等于1 下降到约等于 0 的一个序列；
            alphas_cumprod = torch.cos(((x / timesteps) + s) / (1 + s) * math.pi * 0.5) ** 2
            alphas_cumprod = alphas_cumprod / alphas_cumprod[0]
            # 开始位置为 1 后面的是 0
            betas = 1 - alphas_cumprod[1:-1]  # 从 1 到 T+1 一共是 T+1 个元素；
            # 从近似为0
            return betas
        ...
        def get_sigmas(thetas):  # 使用对应的漂移的强度获取到扩散的强度；；
        ...
        if schedule == 'cosine':
            ...
            thetas = constant_theta_schedule(T)  # betas
        else:
        ...
        self.dt = -1 / thetas_cumsum[-1] * math.log(eps)  # 这里已经提前定义为了 -1/T
        sigma_bars = get_sigma_bars(thetas_cumsum)

        self.thetas = thetas.to(self.device)
        self.sigmas = sigmas.to(self.device)  # 都是随着时间加速增长变化的；
    ...
    def score_fn(self, x, t, **kwargs):
        ...
        noise = self.model(x, self.mu, t, **kwargs)  # x 需要的均值 时间内容 Unet 对其进行预测；
